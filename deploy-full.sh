...
# Redirectionのルール IDを取得
echo "🔍 RedirectionルールIDを取得中..."
NONCE=$(curl -s -b "$COOKIE_JAR" "$WP_URL/wp-admin/tools.php?page=redirection.php" | grep -o '"nonce":"[^"]*"' | head -1 | cut -d'"' -f4)
REDIRECT_DATA=$(curl -s -b "$COOKIE_JAR" \
  "$WP_URL/wp-admin/admin-ajax.php" \
  -d "action=redirection_ajax&_wpnonce=$NONCE&path=%2Fred%2F&sub=redirect&action-type=list&filterby=&filter=&per_page=25&page=1")
REDIRECT_ID=$(echo "$REDIRECT_DATA" | python3 -c "import sys,json; data=json.load(sys.stdin); items=data.get('items',[]); print(items[0]['id'] if items else '')" 2>/dev/null || echo "")
...
if [ -z "$REDIRECT_ID" ]; then
  ...
  curl -s -b "$COOKIE_JAR" \
    "$WP_URL/wp-admin/admin-ajax.php" \
    -d "action=redirection_ajax&_wpnonce=$NONCE&path=%2Fred%2F&sub=redirect&action-type=disable&id[]=$REDIRECT_ID" > /dev/null
...
if [ -z "$ZIP_URL" ]; then
  ...
  # Redirectionを戻す
  [ -n "$REDIRECT_ID" ] && curl -s -b "$COOKIE_JAR" "$WP_URL/wp-admin/admin-ajax.php" \
    -d "action=redirection_ajax&_wpnonce=$NONCE&path=%2Fred%2F&sub=redirect&action-type=enable&id[]=$REDIRECT_ID" > /dev/null
  exit 1
fi
